...
def tasks_iter():
	...
	logging.debug("systemtools: user_tasks_folder is %s" % user_tasks_folder)
	...
	for fn in os.listdir(built_in_tasks_folder): # user_tasks_folder):
		if fn.startswith('Task.') and not skip_pattern.search(fn):
			fn = os.path.join(built_in_tasks_folder, fn)
			if os.path.isdir(fn):
