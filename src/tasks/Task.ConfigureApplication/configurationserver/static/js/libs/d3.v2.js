(function() {
  ...
  d3.layout.force = function() {
    ...
    var force = {}, event = d3.dispatch("start", "tick", "end"), size = [ 1, 1 ], drag, alpha, friction = .9, linkDistance = d3_layout_forceLinkDistance, linkStrength = d3_layout_forceLinkStrength, charge = -30, gravity = .1, gravityOrigin, theta = .8, interval, nodes = [], links = [], distances, strengths, charges;
    force.tick = function() {
      if ((alpha *= .99) < .005) {
        event.end({
          type: "end",
      ...
      , gravityOriginFn, gravityOriginValues, gravityFn, undefined
      for (i = 0; i < m; ++i) {
        o = links[i];
        s = o.source;
        t = o.target;
      ...
      if (alpha){
        if (gravityOrigin) {
          if (typeof gravityOrigin === 'function') {
            gravityOriginFn = gravityOrigin
          } else {
            gravityOriginFn = function(){return gravityOrigin}
          }
        } else {
          gravityOriginFn = function(){return {
            'x': size[0] / 2
            ,'y': size[1] / 2
          }}
        }
        ...
        if (typeof gravity === 'function') {
          ...
          gravityFn = function(){return gravity}
        }
        ...
        if (alpha) while (++i < n) {
          o = nodes[i]
          if (k = alpha * gravity.call(this, o, i)){
            gravityOriginValues = gravityOriginFn.call(this, o, i)
            if (gravityOriginValues.x !== undefined) {
              o.x += (gravityOriginValues.x - o.x) * k;
            }
            if (gravityOriginValues.y !== undefined) {
              o.y += (gravityOriginValues.y - o.y) * k;
            }
          }
        }
      }
    ...
    force.gravityOrigin = function(x) {
      if (!arguments.length) return gravityOrigin;
      gravityOrigin = x;
      return force;
    };
